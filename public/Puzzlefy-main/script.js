...
const Theme = {
    background: [ 255, 187, 187 ],  // #FFBBBB - Color base del gradiente
    board: [ 240 ],
    board_fill: [ 235 ],
    board_outline: [ 220 ]
}
...
function setup() {
    const imageURL = params.get('image') || '/love-return/puzzle/rompecabezas.jpg';
...
function start() {
    ...
    // setupLinks(); // This line is removed as per the edit hint.
...
function draw() {
    ...
    // Crear gradiente con los colores rosados
    let grad = drawingContext.createLinearGradient(0, 0, width, height);
    grad.addColorStop(0, "#FFBBBB");    // Color base (izquierda)
    grad.addColorStop(0.5, "#FFA2A2");  // Color claro (centro)
    grad.addColorStop(1, "#FF8F8F");    // Color más oscuro (derecha)
...
function reset() {
    ...
    // Limpiar piezas colocadas
    placed = []
    unplaced = []
    puzzle.pieces.clear()
    
    // Recrear las piezas en posiciones aleatorias
    pieces = []
    for(var x=0; x<cols; x++) {
        for(var y=0; y<rows; y++) {
            var orientation = Math.random() < 0.5 ? HORIZONTAL : VERTICAL
